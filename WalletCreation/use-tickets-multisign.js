...
async function main() {
  ...
  const signerLiSetSignerList_tx_prepared = await client.autofill(
    signerLiSetSignerList_tx
  );
  const SetSignerList_tx_signed = main_wallet.sign(
    signerLiSetSignerList_tx_prepared
  );
  console.log(`\n SignerListSet Tx hash: ${SetSignerList_tx_signed.hash}`);

  const setsignerlist_submit = await client.submitAndWait(
    SetSignerList_tx_signed.tx_blob
  );
  console.log(
    `\t Submit result: ${setsignerlist_submit.result.meta.TransactionResult}`
  );
  ...
  const ticket_submit = await client.submitAndWait(
    CreateTicket_tx_signed.tx_blob
  );
  console.log(
    "        Submit result:",
    ticket_submit.result.meta.TransactionResult
  );
  ...
  for (let i = 0; i < ticket_response.result.account_objects.length; i++) {
    console.log(
      `Ticket ${i + 1}: ${
        ticket_response.result.account_objects[i].TicketSequence
      }`
    );
  }
  ...
  const Payment_tx_prepared = await client.autofill(
    Payment_tx,
    (signersCount = 3)
  );

  // Each signer will sign the prepared tx (AccountSet_tx) and their signatures will be combines into 1 multi-sig transaction
  const { tx_blob: Payment_tx_signed_1 } = wallet_1.sign(
    Payment_tx_prepared,
    (multisign = true)
  );
  const { tx_blob: Payment_tx_signed_2 } = wallet_2.sign(
    Payment_tx_prepared,
    (multisign = true)
  );
  const { tx_blob: Payment_tx_signed_3 } = wallet_3.sign(
    Payment_tx_prepared,
    (multisign = true)
  );
  ...
  const multisignedTx = xrpl.multisign([
    Payment_tx_signed_1,
    Payment_tx_signed_2,
    Payment_tx_signed_3,
  ]);
  ...
  console.log(
    "\n Multi-sig Submit result:",
    multisig_submit.result.meta.TransactionResult
  );
